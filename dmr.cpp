...
const uint32_t ENCODING_TABLE_1676[] = {
    0x0000U, 0x0273U, 0x04E5U, 0x0696U, 0x09C9U, 0x0BBAU, 0x0D2CU, 0x0F5FU,
    0x11E2U, 0x1391U, 0x1507U, 0x1774U, 0x182BU, 0x1A58U, 0x1CCEU, 0x1EBDU,
    0x21B7U, 0x23C4U, 0x2552U, 0x2721U, 0x287EU, 0x2A0DU, 0x2C9BU, 0x2EE8U,
    0x3055U, 0x3226U, 0x34B0U, 0x36C3U, 0x399CU, 0x3BEFU, 0x3D79U, 0x3F0AU,
    0x411EU, 0x436DU, 0x45FBU, 0x4788U, 0x48D7U, 0x4AA4U, 0x4C32U, 0x4E41U,
    0x50FCU, 0x528FU, 0x5419U, 0x566AU, 0x5935U, 0x5B46U, 0x5DD0U, 0x5FA3U,
    0x60A9U, 0x62DAU, 0x644CU, 0x663FU, 0x6960U, 0x6B13U, 0x6D85U, 0x6FF6U,
    0x714BU, 0x7338U, 0x75AEU, 0x77DDU, 0x7882U, 0x7AF1U, 0x7C67U, 0x7E14U,
    0x804FU, 0x823CU, 0x84AAU, 0x86D9U, 0x8986U, 0x8BF5U, 0x8D63U, 0x8F10U,
    0x91ADU, 0x93DEU, 0x9548U, 0x973BU, 0x9864U, 0x9A17U, 0x9C81U, 0x9EF2U,
    0xA1F8U, 0xA38BU, 0xA51DU, 0xA76EU, 0xA831U, 0xAA42U, 0xACD4U, 0xAEA7U,
    0xB01AU, 0xB269U, 0xB4FFU, 0xB68CU, 0xB9D3U, 0xBBA0U, 0xBD36U, 0xBF45U,
    0xC151U, 0xC322U, 0xC5B4U, 0xC7C7U, 0xC898U, 0xCAEBU, 0xCC7DU, 0xCE0EU,
    0xD0B3U, 0xD2C0U, 0xD456U, 0xD625U, 0xD97AU, 0xDB09U, 0xDD9FU, 0xDFECU,
    0xE0E6U, 0xE295U, 0xE403U, 0xE670U, 0xE92FU, 0xEB5CU, 0xEDCAU, 0xEFB9U,
...
DMR::DMR()
  ...
  // Connect Network Manager signal
...
void DMR::set_dmr_params(uint8_t essid, QString password, QString lat,
                         QString lon, QString location, QString desc,
                         QString freq, QString url, QString swid, QString pkid,
                         QString options) {
  if (essid) {
...
void DMR::process_udp() {
  ...
  if ((m_modeinfo.status != CONNECTED_RW) &&
      (::memcmp(buf.data() + 3, "NAK", 3U) == 0)) {
    m_modeinfo.status = DISCONNECTED;
  }
  if ((m_modeinfo.status != CONNECTED_RW) &&
      (::memcmp(buf.data(), "MSTCL", 5U) == 0)) {
    m_modeinfo.status = CLOSED;
  }
  if ((m_modeinfo.status != CONNECTED_RW) &&
      (::memcmp(buf.data(), "RPTACK", 6U) == 0)) {
    ...
    case CONNECTING:
      ...
      sha256.buffer((uint8_t *)in.data(),
                    (uint32_t)(m_password.size() + sizeof(uint32_t)),
                    (uint8_t *)out.data() + 8U);
      break;
    case DMR_AUTH:
      out.clear();
      ...
      ::sprintf(buffer + 8U,
                "%-8.8s%09u%09u%02u%02u%8.8s%9.9s%03d%-20.20s%-19.19s%c%-124."
                "124s%-40.40s%-40.40s",
                m_modeinfo.callsign.toStdString().c_str(), m_freq.toUInt(),
                m_freq.toUInt(), 1, 1, latitude, longitude, 0,
                m_location.toStdString().c_str(), m_desc.toStdString().c_str(),
                '4', m_url.toStdString().c_str(), m_swid.toStdString().c_str(),
                m_pkid.toStdString().c_str());
  ...
  if ((buf.size() != 55) && ((m_modeinfo.stream_state == STREAM_LOST) ||
                             (m_modeinfo.stream_state == STREAM_END))) {
    //  m_modeinfo.stream_state = STREAM_IDLE; fetchFirstName(m_modeinfo.srcid);
    //  // Ensure this line is correctly placed
  }
  if ((buf.size() == 55) && (::memcmp(buf.data(), "DMRD", 4U) == 0) &&
      ((uint8_t)buf.data()[15] & 0x20) && (m_modeinfo.status == CONNECTED_RW)) {
    m_rxwatchdog = 0;
    uint8_t t = 0;
    if ((uint8_t)buf.data()[15] & 0x02) {
      ...
      m_modeinfo.ts = QDateTime::currentMSecsSinceEpoch();
      m_modeinfo.srcid =
          (uint32_t)((buf.data()[5] << 16) | ((buf.data()[6] << 8) & 0xff00) |
                     (buf.data()[7] & 0xff));
      m_modeinfo.dstid =
          (uint32_t)((buf.data()[8] << 16) | ((buf.data()[9] << 8) & 0xff00) |
                     (buf.data()[10] & 0xff));
      m_modeinfo.gwid = (uint32_t)((buf.data()[11] << 24) |
                                   ((buf.data()[12] << 16) & 0xff0000) |
                                   ((buf.data()[13] << 8) & 0xff00) |
                                   (buf.data()[14] & 0xff));
      m_modeinfo.streamid = (uint32_t)((buf.data()[16] << 24) |
                                       ((buf.data()[17] << 16) & 0xff0000) |
                                       ((buf.data()[18] << 8) & 0xff00) |
                                       (buf.data()[19] & 0xff));
      ...
      /* if (m_currentSrcId != m_modeinfo.srcid) {
       ...
       qDebug() << "New DMR stream from " << m_modeinfo.srcid << " to " <<
       m_modeinfo.dstid;
       // Call fetchFirstName with the source ID from the incoming stream
                   qDebug() << "Calling fetchFirstName with srcId:" <<
       m_modeinfo.srcid;
                   fetchFirstName(m_modeinfo.srcid);  // Add this line to fetch
       the name on receiving */
    }
    if (m_modem) {
  ...
  if ((buf.size() == 55) && (::memcmp(buf.data(), "DMRD", 4U) == 0) &&
    ...
    if (!m_tx && ((m_modeinfo.stream_state == STREAM_LOST) ||
                  (m_modeinfo.stream_state == STREAM_END) ||
    ...
    m_modeinfo.gwid =
        (uint32_t)((buf.data()[11] << 24) |
                   ((buf.data()[12] << 16) & 0xff0000) |
                   ((buf.data()[13] << 8) & 0xff00) | (buf.data()[14] & 0xff));
    m_modeinfo.streamid =
        (uint32_t)((buf.data()[16] << 24) |
                   ((buf.data()[17] << 16) & 0xff0000) |
                   ((buf.data()[18] << 8) & 0xff00) | (buf.data()[19] & 0xff));
    m_modeinfo.frame_number = (uint8_t)buf.data()[4];
    ...
    // uint32_t id = (uint32_t)((buf.data()[5] << 16) | ((buf.data()[6] << 8) &
    // 0xff00) | (buf.data()[7] & 0xff));
  }
...
void DMR::process_modem_data(QByteArray d) {
  ...
  if ((p_frame[3U] & DMR_SYNC_DATA) == DMR_SYNC_DATA) {
    if ((m_dataType == DT_VOICE_LC_HEADER) &&
        (m_modeinfo.stream_state == STREAM_IDLE)) {
  ...
  if (m_debug) {
    ...
    for (int i = 0; i < txdata.size(); ++i) {
      s +=
          " " + QString("%1").arg((uint8_t)txdata.data()[i], 2, 16, QChar('0'));
    }
    debug << s;
  }
}
...
void DMR::build_frame() {
  // qDebug() << "DMR: slot:cc:flco == " << m_txslot << ":" << m_txcc << ":" <<
  // m_flco;
  m_dmrFrame[0U] = 'D';
  m_dmrFrame[1U] = 'M';
  m_dmrFrame[2U] = 'R';
...
void DMR::encode_embedded_data() {
  ...
  for (uint32_t a = 0U; a < 16U; a++)
    data[a + 112U] = data[a + 0U] ^ data[a + 16U] ^ data[a + 32U] ^
                     data[a + 48U] ^ data[a + 64U] ^ data[a + 80U] ^
                     data[a + 96U];
...
void DMR::get_slot_data(uint8_t *data) {
  ...
  data[13U] = (data[13U] & 0x0FU) | ((DMRSlotType[0U] << 6) & 0xC0U) |
              ((DMRSlotType[1U] >> 2) & 0x30U);
  data[19U] = (data[19U] & 0xF0U) | ((DMRSlotType[1U] >> 2) & 0x0FU);
  data[20U] = (data[20U] & 0x03U) | ((DMRSlotType[1U] << 6) & 0xC0U) |
              ((DMRSlotType[2U] >> 2) & 0x3CU);
}
...
void DMR::addDMRAudioSync(uint8_t *data, bool duplex) {
  if (duplex) {
    for (uint32_t i = 0U; i < 7U; i++)
      data[i + 13U] =
          (data[i + 13U] & ~SYNC_MASK[i]) | BS_SOURCED_AUDIO_SYNC[i];
  } else {
    for (uint32_t i = 0U; i < 7U; i++)
      data[i + 13U] =
          (data[i + 13U] & ~SYNC_MASK[i]) | MS_SOURCED_AUDIO_SYNC[i];
  }
}
...
void DMR::process_rx_data() {
  ...
  } else if (((m_modeinfo.stream_state == STREAM_END) ||
              (m_modeinfo.stream_state == STREAM_LOST)) &&
             (m_rxmodemq.size() < 50)) {
    m_rxtimer->stop();
