...
DroidStar::DroidStar(QObject *parent)
  ...
  /*
   if (!connect(networkManager, &QNetworkAccessManager::finished, this,
   &DroidStar::onNetworkReply)) { qDebug() << "Signal connection failed"; } else
   { qDebug() << "Signal connected successfully"; qDebug() << "Network Manager
   initialized.";
   }

   */

  // if (connect(dmr, &DMR::firstNameChanged, this, &DroidStar::updateFirstName,
  // Qt::QueuedConnection)) {
  ...
  connect(m_USBmonitor, SIGNAL(devices_changed()), this,
          SLOT(discover_devices()));
#endif
  check_host_files();
  discover_devices();
  process_settings();
...
void DroidStar::fetchFirstNameFromDMR(unsigned int srcid) {
    if (srcid && srcid != lastSrcId) {
        lastSrcId = srcid;
        QUrl url("https://radioid.net/api/dmr/user/?id=" +
QString::number(srcid)); QNetworkRequest request(url);

        connect(networkManager, &QNetworkAccessManager::finished, this,
[this](QNetworkReply *reply) { if (reply->error() == QNetworkReply::NoError) {
                ...
                if (!json.isNull()) {
                    ...
                    if (!results.isEmpty()) {
                        ...
                        //vuidUpdater->setFirstName(firstName);  // Use the
member variable
                    }
                }
            }
    ...
    if (reply->error() == QNetworkReply::NoError) {
        ...
        if (!json.isNull()) {
            ...
            if (!results.isEmpty()) {
                ...
                QString firstName = firstResult["fname"].toString();
                qDebug() << "First name fetched from API:" << firstName;
                //vuidUpdater->setFirstName(firstName);  // Assuming
setFirstName() updates m_firstName and emits firstNameChanged emit
...
void DroidStar::keepScreenOn() {
  ...
  if (qApp->checkPermission(microphonePermission) !=
      Qt::PermissionStatus::Granted) {
    qApp->requestPermission(microphonePermission, this,
                            &DroidStar::keepScreenOn);
  }
}
...
void DroidStar::download_file(QString f, bool u) {
  ...
  if (u) {
    connect(http, SIGNAL(file_downloaded(QString)), this,
            SLOT(url_downloaded(QString)));
  } else {
    connect(http, SIGNAL(file_downloaded(QString)), this,
            SLOT(file_downloaded(QString)));
  }
...
void DroidStar::process_connect() {
  if (connect_status != Mode::DISCONNECTED) {
    ...
    if ((m_protocol == "M17") && !m_mdirect && (m_ipv6) && (sl.size() > 2) &&
        (sl.at(2) != "none")) {
    ...
    if (m_protocol == "IAX") {
      ...
      connect(this, SIGNAL(send_dtmf(QByteArray)), m_mode,
              SLOT(send_dtmf(QByteArray)));
    }

    m_mode->init(m_callsign, m_dmrid, nxdnid, m_module, m_refname, m_host,
                 m_port, m_ipv6, vocoder, modem, m_capture, m_playback,
                 m_mdirect);
    m_mode->set_modem_flags(rxInvert, txInvert, pttInvert, useCOSAsLockout,
                            duplex);
    m_mode->set_modem_params(
        m_modemBaud.toUInt(), rxfreq, txfreq, m_modemTxDelay.toInt(),
        m_modemRxLevel.toFloat(), m_modemRFLevel.toFloat(), ysfTXHang,
        m_modemCWIdTxLevel.toFloat(), m_modemDstarTxLevel.toFloat(),
        m_modemDMRTxLevel.toFloat(), m_modemYSFTxLevel.toFloat(),
        m_modemP25TxLevel.toFloat(), m_modemNXDNTxLevel.toFloat(),
        pocsagTXLevel, m17TXLevel);

    connect(this, SIGNAL(module_changed(char)), m_mode,
            SLOT(module_changed(char)));
    connect(m_mode, SIGNAL(update(Mode::MODEINFO)), this,
            SLOT(update_data(Mode::MODEINFO)));
    connect(m_mode, SIGNAL(update_log(QString)), this,
            SLOT(updatelog(QString)));
    connect(m_mode, SIGNAL(update_output_level(unsigned short)), this,
            SLOT(update_output_level(unsigned short)));
    ...
    connect(this, SIGNAL(input_source_changed(int, QString)), m_mode,
            SLOT(input_src_changed(int, QString)));
    connect(this, SIGNAL(swrx_state_changed(int)), m_mode,
            SLOT(swrx_state_changed(int)));
    connect(this, SIGNAL(swtx_state_changed(int)), m_mode,
            SLOT(swtx_state_changed(int)));
    connect(this, SIGNAL(agc_state_changed(int)), m_mode,
            SLOT(agc_state_changed(int)));
    ...
    connect(this, SIGNAL(in_audio_vol_changed(qreal)), m_mode,
            SLOT(in_audio_vol_changed(qreal)));
    connect(this, SIGNAL(mycall_changed(QString)), m_mode,
            SLOT(mycall_changed(QString)));
    connect(this, SIGNAL(urcall_changed(QString)), m_mode,
            SLOT(urcall_changed(QString)));
    connect(this, SIGNAL(rptr1_changed(QString)), m_mode,
            SLOT(rptr1_changed(QString)));
    connect(this, SIGNAL(rptr2_changed(QString)), m_mode,
            SLOT(rptr2_changed(QString)));
    connect(this, SIGNAL(usrtxt_changed(QString)), m_mode,
            SLOT(usrtxt_changed(QString)));
    connect(this, SIGNAL(debug_changed(bool)), m_mode,
            SLOT(debug_changed(bool)));
    ...
    if (m_protocol == "DMR") {
      ...
      m_mode->set_dmr_params(m_essid, dmrpass, m_latitude, m_longitude,
                             m_location, m_description, m_freq, m_url, m_swid,
                             m_pkgid, m_dmropts);
      connect(this, SIGNAL(dmr_tgid_changed(int)), m_mode,
              SLOT(dmr_tgid_changed(int)));
      connect(this, SIGNAL(dmrpc_state_changed(int)), m_mode,
              SLOT(dmrpc_state_changed(int)));
    ...
    if (m_protocol == "M17") {
      connect(this, SIGNAL(m17_rate_changed(int)), m_mode,
              SLOT(rate_changed(int)));
      connect(this, SIGNAL(m17_can_changed(int)), m_mode,
              SLOT(can_changed(int)));
      if (m_mdirect) {
        connect(this, SIGNAL(dst_changed(QString)), m_mode,
                SLOT(dst_changed(QString)));
      }
    }
  ...
  /*
      ...
      qDebug() << "process_connect called m_tgif_password == " <<
     m_tgif_password; qDebug() << "process_connect called m_dmropts == " <<
     m_dmropts; qDebug() << "process_connect called m_refname == " << m_refname;
...
void DroidStar::process_settings() {
  m_ipv6 = (m_settings->value("IPV6").toString().simplified() == "true")
               ? true
               : false;
  ...
  m_pkgid = m_settings->value("DMRPKGID", "MMDVM_MMDVM_HS_Hat")
                .toString()
                .simplified();
  m_dmropts = m_settings->value("DMROPTS").toString().simplified();
  m_dmr_destid =
      m_settings->value("DMRTGID", "4000").toString().simplified().toUInt();
  ...
  m_txtimeout =
      m_settings->value("TXTIMEOUT", "300").toString().simplified().toUInt();
  m_toggletx =
      (m_settings->value("TXTOGGLE", "true").toString().simplified() == "true")
          ? true
          : false;
  m_dstarusertxt = m_settings->value("USRTXT").toString().simplified();
  m_xrf2ref = (m_settings->value("XRF2REF").toString().simplified() == "true")
                  ? true
                  : false;
  m_localhosts = m_settings->value("LOCALHOSTS").toString();

  m_modemRxFreq =
      m_settings->value("ModemRxFreq", "438800000").toString().simplified();
  m_modemTxFreq =
      m_settings->value("ModemTxFreq", "438800000").toString().simplified();
  m_modemRxOffset =
      m_settings->value("ModemRxOffset", "0").toString().simplified();
  m_modemTxOffset =
      m_settings->value("ModemTxOffset", "0").toString().simplified();
  m_modemRxDCOffset =
      m_settings->value("ModemRxDCOffset", "0").toString().simplified();
  m_modemTxDCOffset =
      m_settings->value("ModemTxDCOffset", "0").toString().simplified();
  m_modemRxLevel =
      m_settings->value("ModemRxLevel", "50").toString().simplified();
  m_modemTxLevel =
      m_settings->value("ModemTxLevel", "50").toString().simplified();
  m_modemRFLevel =
      m_settings->value("ModemRFLevel", "100").toString().simplified();
  m_modemTxDelay =
      m_settings->value("ModemTxDelay", "100").toString().simplified();
  m_modemCWIdTxLevel =
      m_settings->value("ModemCWIdTxLevel", "50").toString().simplified();
  m_modemDstarTxLevel =
      m_settings->value("ModemDstarTxLevel", "50").toString().simplified();
  m_modemDMRTxLevel =
      m_settings->value("ModemDMRTxLevel", "50").toString().simplified();
  m_modemYSFTxLevel =
      m_settings->value("ModemYSFTxLevel", "50").toString().simplified();
  m_modemP25TxLevel =
      m_settings->value("ModemP25TxLevel", "50").toString().simplified();
  m_modemNXDNTxLevel =
      m_settings->value("ModemNXDNTxLevel", "50").toString().simplified();
  m_modemBaud =
      m_settings->value("ModemBaud", "115200").toString().simplified();
  m_modemM17CAN = m_settings->value("ModemM17CAN", "0").toString().simplified();
  m_modemTxInvert =
      (m_settings->value("ModemTxInvert", "true").toString().simplified() ==
       "true")
          ? true
          : false;
  m_modemRxInvert =
      (m_settings->value("ModemRxInvert", "false").toString().simplified() ==
       "true")
          ? true
          : false;
  m_modemPTTInvert =
      (m_settings->value("ModemPTTInvert", "false").toString().simplified() ==
       "true")
          ? true
          : false;
  ...
  if (check_file.exists() && check_file.isFile()) {
    ...
    if (f.open(QIODevice::ReadOnly)) {
      ...
      for (const auto &i : std::as_const(m_customhosts)) {
        ...
        if (line.at(0) == m) {
          m_hostmap[line.at(1).simplified()] =
              line.at(2).simplified() + "," + line.at(3).simplified();
        }
      }
      ...
      while (!f.atEnd()) {
        ...
        if (ll.size() > 4) {
          if (ll.at(1).simplified() != "nn") {
            m_hostmap[ll.at(0).simplified() + " - " + ll.at(1).simplified()] =
                ll.at(2).left(6).toLower() + ".xreflector.net,62500";
          }
        }
      }
  ...
  if (check_file.exists() && check_file.isFile()) {
    ...
    if (f.open(QIODevice::ReadOnly)) {
      while (!f.atEnd()) {
        ...
        if (ll.size() > 4) {
          if ((ll.at(0).simplified() != "DMRGateway") &&
            ...
            m_hostmap[ll.at(0).simplified()] =
                ll.at(2) + "," + ll.at(4) + "," + ll.at(3);
          }
        }
      }
      ...
      for (const auto &i : std::as_const(m_customhosts)) {
        ...
        if (line.at(0) == "DMR") {
          m_hostmap[line.at(1).simplified()] = line.at(2).simplified() + "," +
                                               line.at(3).simplified() + "," +
                                               line.at(4).simplified();
        }
      }
...
void DroidStar::process_iax_hosts() {
  ...
  for (const auto &i : std::as_const(m_customhosts)) {
    ...
    if (line.at(0) == "IAX") {
      m_hostmap[line.at(1).simplified()] =
          line.at(2).simplified() + "," + line.at(3).simplified() + "," +
          line.at(4).simplified() + "," + line.at(5).simplified();
    }
  }
...
void DroidStar::check_host_files() {
  ...
  /*
  #if defined(Q_OS_ANDROID)
      QString vocname = "/vocoder_plugin." + QSysInfo::productType() + "." +
  QSysInfo::currentCpuArchitecture(); #else QString vocname = "/vocoder_plugin."
  + QSysInfo::kernelType() + "." + QSysInfo::currentCpuArchitecture(); #endif
      QString newvoc =
  QStandardPaths::writableLocation(QStandardPaths::DownloadLocation) + vocname;
...
void DroidStar::update_data(Mode::MODEINFO info) {
  if ((connect_status == Mode::CONNECTING) &&
      (info.status == Mode::DISCONNECTED)) {
    process_connect();
    return;
  }

  if ((connect_status == Mode::CONNECTING) &&
      (info.status == Mode::CONNECTED_RW)) {
    ...
    if (m_mycall.isEmpty())
      set_mycall(m_callsign);
    if (m_urcall.isEmpty())
      set_urcall("CQCQCQ");
    if (m_rptr1.isEmpty())
      set_rptr1(m_callsign + " " + m_module);
    emit update_log("Connected to " + m_protocol + " " + m_refname + " " +
...
*/
  ...
  m_ambestatustxt =
      "AMBE: " + (info.ambeprodid.isEmpty() ? "No device" : info.ambeprodid);
  ...
  if (verlist.size() > 7) {
    m_ambestatustxt +=
        " " + verlist.at(0) + " " + verlist.at(5) + " " + verlist.at(6);
  }
  ...
  if (info.stream_state == Mode::STREAM_IDLE) {
    ...
    m_data5.clear();
    m_data6.clear();
  } else if (m_protocol == "REF" || m_protocol == "XRF" ||
             m_protocol == "DCS") {
    m_data1 = info.src;
    m_data2 = info.dst;
    m_data3 = info.gw;
    ...
    m_data5 = QString::number(info.streamid, 16) + " " +
              QString("%1").arg(info.frame_number, 2, 16, QChar('0'));
    ...
    if (info.type >= 0) {
      ...
      m_data6 = QString::number(info.frame_number) + "/" +
                QString::number(info.frame_total);
    } else {
      m_data5 = m_data6 = "";
    }
  } else if (m_protocol == "DMR") {
  ...
  QString t = QDateTime::fromMSecsSinceEpoch(info.ts).toString(
      "yyyy.MM.dd hh:mm:ss.zzz");
  if ((m_protocol == "DMR") || (m_protocol == "P25") ||
      (m_protocol == "NXDN")) {
    if (info.stream_state == Mode::STREAM_NEW) {
      emit update_log(t + " " + m_protocol + " RX started id: " + " srcid: " +
                      QString::number(info.srcid) +
                      " dstid: " + QString::number(info.dstid));
    }
    if (info.stream_state == Mode::STREAM_END) {
      emit update_log(t + " " + m_protocol + " RX ended id: " + " srcid: " +
                      QString::number(info.srcid) +
                      " dstid: " + QString::number(info.dstid));
    }
    if (info.stream_state == Mode::STREAM_LOST) {
      emit update_log(t + " " + m_protocol + " RX lost id: " + " srcid: " +
                      QString::number(info.srcid) +
                      " dstid: " + QString::number(info.dstid));
    }
  } else {
    if (info.stream_state == Mode::STREAM_NEW) {
      emit update_log(t + " " + m_protocol +
                      " RX started id: " + QString::number(info.streamid, 16) +
                      " src: " + info.src + " dst: " + info.gw2);
    }
    if (info.stream_state == Mode::STREAM_END) {
      emit update_log(t + " " + m_protocol +
                      " RX ended id: " + QString::number(info.streamid, 16) +
                      " src: " + info.src + " dst: " + info.gw2);
    }
    if (info.stream_state == Mode::STREAM_LOST) {
      emit update_log(t + " " + m_protocol +
                      " RX lost id: " + QString::number(info.streamid, 16) +
                      " src: " + info.src + " dst: " + info.gw2);
    }
  }
  emit update_data();
...
void DroidStar::updatelog(QString s) { emit update_log(s); }
...
void DroidStar::press_tx() { emit tx_pressed(); }

void DroidStar::release_tx() { emit tx_released(); }
