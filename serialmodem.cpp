...
SerialModem::~SerialModem() { m_serial->close(); }
...
void SerialModem::set_modem_flags(bool rxInvert, bool txInvert, bool pttInvert,
                                  bool useCOSAsLockout, bool duplex) {
  m_rxInvert = rxInvert;
  m_txInvert = txInvert;
...
void SerialModem::set_modem_params(
    uint32_t baudrate, uint32_t rxfreq, uint32_t txfreq, uint32_t txDelay,
    float rxLevel, float rfLevel, uint32_t ysfTXHang, float cwIdTXLevel,
    float dstarTXLevel, float dmrTXLevel, float ysfTXLevel, float p25TXLevel,
    float nxdnTXLevel, float pocsagTXLevel, float m17TXLevel) {
  m_baudrate = baudrate;
  m_rxfreq = rxfreq;
  m_txfreq = txfreq;
  m_txDelay = txDelay;
...
void SerialModem::process_modem() {
  ...
  if (((uint8_t)m_serialdata[0] == MMDVM_FRAME_START) &&
      (m_serialdata.size() >= m_serialdata[1])) {
